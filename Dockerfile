...
#
# Multi-stage build for Census (people directory, profiles & groups).
#   - builder: rust:1.96-slim (Debian trixie).
#   - runtime: debian:trixie-slim (matching glibc), non-root, ca-certificates.
#
# Like inkwell/cortex, Census links NO OpenSSL: pulldown-cmark is pure Rust and sqlx uses
# `rustls`, so the binary depends only on glibc — no libssl in either stage. The container
# HEALTHCHECK uses the built-in `census healthcheck` subcommand, so no extra HTTP tool is
# needed in the image.
...
RUN useradd --system --uid 10001 --user-group --no-create-home census
COPY --from=builder /build/target/release/census /usr/local/bin/census

USER census
# Default in-container bind; overridable at runtime.
ENV BIND_ADDR=0.0.0.0:9130
EXPOSE 9130

# Dependency-free liveness probe -> GET /healthz on the loopback, exit 0/1.
HEALTHCHECK --interval=10s --timeout=5s --start-period=5s --retries=3 \
    CMD ["census", "healthcheck"]
